...
def ccW12(y, N, blks):
    pcs = []
    for i in range(0, N - blks, blks/2):
...
def locExtremum(dat, ts=50, ys=30, os=0):
    ...
    for t in range(L - ts - 1, L - ts - 2, -ts/3):
        imin = dat[t:t+ts].argmin()
        imax = dat[t:t+ts].argmax()
        ymax = dat[imax + t]
        ymin = dat[imin + t]
...
def downsample_data(dat):
    isis_list = []
    TR = dat.shape[1]

    for tr in range(TR):
        spkts = _N.where(d[:, tr] == 1)[0]
    ...
    evry = _N.where((y[0:-1] < 0.01) & (y[1:] >= 0.01))[0][0] + 1
    ...
    newL = L // evry
    dnew = _N.array(d[0:newL * evry])

    dnew_rs = dnew.reshape((newL, 3, d.shape[1]))
